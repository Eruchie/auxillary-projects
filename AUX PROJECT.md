**AUX PROJECT 1: SHELL SCRIPTING**
___

In this project, onboard 20 new Linux users onto a server. Create a shell script that reads a `csv` file that contains the firstname of users to be onboarded.

1. On the Server, create a project folder called `Shell` and in the `Shell` folder create a `csv` and `sh` files. Also, create a group,`developers` where the users will be added.
   - `mkdir Shell && cd Shell`
   - `vi names.csv`
     - Insert 20 random names, one name per line.
    - `vi scripts.sh`
      - Paste the script that will read names of users on the `csv` file and create them on the server. Ensure the script check for existence of the user on  the system before creating it. Also, ensure each user is created with a default `home` folder.
     - `sudo groupadd developers` 

1. On the `Shell` directory, make the script executable and then deploy.
     - `sudo chmod +X scripts.sh`
     - `sudo ./scripts.sh`

1. Create `.ssh` folder for each user within its `home` folder and also create a file `authorized_keys` and paste the public key.
   - `mkdir .ssh && cd .ssh`
   - `vi authorized_keys`
   - Paste the public key below
   
     ```
     ssh-rsa 
     ```
1. On the client Server user `home` directory, navigate to `.ssh` folder, create public and private key files.

    - `cd .ssh`
    - `vi id_rsa`
    - Paste the private key below and save.
        ```
        -----BEGIN OPENSSH PRIVATE KEY-----
        
        -----END OPENSSH PRIVATE KEY-----
        ```
     - `sudo chmod 600 id_rsa`
         - Set permission.
     - `vi id_rsa.pub`
     - Paste the public key below and save.

        ```
        ssh-rsa 
        ```
1. Test a few of the users randomly, and ensure that you are able to connect to the Server using the private key and the public key.